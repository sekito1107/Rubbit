...
export class CursorDocComponent {
  ...
  private renderContextualUI(type: string, methods: any[]): void {
    ...
    summary.className =
      "flex items-center cursor-pointer p-2 list-none text-xs font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-white/5 rounded";
  ...
  private createContextualMethodCard(item: any): HTMLElement {
    ...
    item.links.forEach((linkInfo: any) => {
      ...
      linkNode.querySelector('[data-role="separatorMethod"]')!.textContent =
        linkInfo.separator + linkInfo.methodName;
